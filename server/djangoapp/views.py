...
def about(request):
    '''Displays company's mission and responsibilities'''
    context = {}
...
def contact(request):
    """Displays contact info to potential clients"""
    context = {}
    if request.method == 'GET':
...
def login_request(request):
    '''Authenicates users' login requests'''
    context = {}
...
def logout_request(request):
    '''logout users if authenticated and return to home page.'''
...
def registration_request(request):
    '''Collect users' info for signup purposes only.'''
    context = {}
...
def get_dealerships(request):
    '''Display cars dealership info for the users.'''
    #url = ''
...
def add_review(request, dealer_id):
    '''Displays dealership reviews and helps users to write reviews about dealership.'''
    if request.method == "GET":
        dealersid = dealer_id
        # url = "https://5b93346d.us-south.apigw.appdomain.cloud/dealerships/dealer-get?dealerId={0}".format(dealersid)
        # Get dealers from the URL
        #context = {"cars": models.CarModel.objects.all(),
